...
pacman::p_load(tidyverse, DT, readxl,
             knitr,patchwork, scales,ggthemes,
...
plot_cpi_time_series <- function(data,
                                 ...
                                 source_note = "<b>Source:</b> CEIC Database; Index, 2024 = 100",
                                 return_plot = FALSE) {
  ...
  if (interactive) {
    ...
    p <- p %>%
      plotly::layout(
        ...
        margin = list(b = if (add_source_note) 120 else 80)
      )
  }
...
Each of the 10 faceted panels reveals distinct patterns. Transport has the widest range (78–105) and most volatility. Clothing & Footwear fluctuates around 92–104 with visible oscillations. Education shows a staircase-like pattern with sudden jumps. HDS and MG&S display gradual upward curves. InfoComm shows a narrow range (96–100). Health rises from 90 to 105. Housing & Utilities climbs steadily from 85 to 100. Food and RSC show smooth upward trajectories.
...
The Food category consists of two main subgroups: **Food Excluding Food and Beverage Serving Services (FFBSS)** with a weight of 651, and **Food and Beverage Serving Services (FBSS)** with a weight of 1,391.
...
The FFBSS subgroup is further divided into **10** detailed classes, including - Rice and Cereals (122), - Meat (101), - Fish and Seafood (79), - Milk, Other Dairy Products and Eggs (64), - Oils and Fats (12), - Fruits and Nuts (79), - Vegetables (82), - Sugar, Confectionery and Desserts (27), - Ready Made and Other Food Products (44), - and Non Alcoholic Beverages (41).
...
The four faceted panels show each FBSS class individually. All display upward trends but differ in steepness and smoothness. Fast Food shows the steepest incline. Hawker Food and Restaurants show similar moderate slopes. Other Catering Services has the flattest trajectory, with most growth occurring 2022–2024. All four series exhibit minimal within-year volatility.
...
The cycle plot shows overall CPI by month (Jan–Dec), with separate lines for each year (2020–2025). Year-over-year levels increase consistently from 2020 (lowest, 85) to 2025 (highest, 101–102). Within each year, CPI values tend to be slightly lower in April–May and higher in November–December. The gap between consecutive years is largest between 2021 and 2022, indicating the fastest year-over-year inflation during that period.
...
Three boxplot panels show CPI distribution by month, quarter, and year. By month: medians gradually increase from January (92) to December (96), with similar interquartile ranges across months. By quarter: Q3 and Q4 medians are slightly higher than Q1 and Q2. By year: the boxes shift upward progressively from 2020 (85) to 2025 (101), with 2022 showing the widest spread. The 2024 box is notably tight with a single outlier.
...
The heatmap uses viridis colour scale across 10 major groups (rows) and dates (columns). Transport stands out with bright yellow/green tiles in early 2020 (lowest CPI 78), transitioning to dark purple by 2023–2025 (highest CPI 100+). Education and Clothing & Footwear show relatively dark purple throughout, indicating consistently higher index values. Food and Housing transition from lighter to darker shades over time. The visual gradient from left to right confirms general upward CPI movement across all groups.
...
```{r}
plot_cpi_acf(cpi_h, plot_level = 0)
```
...
```{r}
plot_cpi_acf(cpi_h, plot_level = 2, category = "Clothing and Footwear (C&F)")
```
...
One limitation of this analysis is that it focuses mainly on exploratory techniques. Future research could incorporate **external** explanatory variables, such as foreign exchange rates, to further investigate potential relationships and patterns in CPI movements.

Overall, this study demonstrates how publicly available statistical data can be transformed into useful insights that support a clearer understanding of CPI trends.


## Shiny App Prototype

In this section, we create several design sketches for the group project to support the implementation of the **Shiny application**. Due to limited drawing skills, the final illustrations were generated with the assistance of AI tools, based on initial **hand drawn sketches** prepared by human.
