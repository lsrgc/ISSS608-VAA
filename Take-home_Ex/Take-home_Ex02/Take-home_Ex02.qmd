...
```{r}
#| eval: false
write_rds(cpi_h, "data/cpi_h.rds")
```
...
plot_cpi_acf <- function(data,
  ...
  # Return the plot object - let R Markdown/Quarto handle the printing
  return(p)
}
```
...
Overall, Singapore CPI series exhibit an upward trend over the study period. Examining the CPI at more detailed category levels reveals that movements in individual components may be masked by the aggregated index. For instance, while the overall **Health CPI** increases moderately from around 89 in 2020 to about 105 in 2025 **(18% increase)**, **Health Insurance** shows a much stronger rise, increasing from approximately 86 to around 122 **(about 41% increase)** over the same period. This sharper increase may be less visible in the aggregate index because categories such as **Outpatient Services** and **Inpatient Services**, which carry larger weights, exhibit more stable price movements. This suggests that aggregate CPI measures may understate certain sector specific cost pressures faced by households. Understanding these underlying dynamics can provide useful insights for both household budgeting decisions and policy considerations.

In terms of **seasonality**, many CPI series show slightly higher median values in Q4, suggesting stronger price levels toward the end of the year. However, this pattern is not consistent across all CPI categories. For example, the **Household Appliances** series does not display a strong Q4 effect; instead, median prices remain relatively stable across quarters, with greater variability observed in January. This indicates that seasonal dynamics may vary across CPI components and can become clearer when analysing more granular categories rather than only the major group index.
...
One limitation of this analysis is that it focuses primarily on examining the CPI series itself. Future research could incorporate **external** variables, such as foreign exchange rates, global supply chain stress indicators, or trade policies. For example, the **reciprocal tariffs** imposed during the Trump administration in 2025 may influence import prices and subsequently affect domestic inflation. Including such macroeconomic factors could help further investigate the underlying drivers of CPI movements and improve the explanatory power of the analysis.
