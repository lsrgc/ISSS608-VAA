...
create_cpi_plot <- function(data, 
  ...
  if (plot_level == 0) {
    ...
    ref_data <- NULL
    ref_label <- NULL
    default_title <- "CPI Aggregated Indices"
    color_label <- "Index"
  ...
  if (is.null(plot_title)) {
    plot_title <- paste0(default_title, ", ", format(start_date, "%Y"), "-", format(end_date, "%Y"))
  }
  ...
  p <- p + labs(
    title = plot_title,
    subtitle = ifelse(has_reference, paste("Black line =", ref_label), NULL),
    x = "Date", y = "CPI Index", color = color_label
  ) +
  scale_x_date(limits = c(start_date, end_date),
               breaks = seq(start_date, end_date, by = "6 months"),
               date_labels = "%b%y",
  ...
  ggplotly(p) %>%
    layout(
      title = list(text = plot_title, font = list(size = 16, family = "Arial", color = "#2c3e50"),
                   x = 0.5, xanchor = "center"),
      legend = list(orientation = "h", y = -0.3, x = 0.5, xanchor = "center",
                    font = list(family = "Arial", size = 9, color = "#333333"),
                    itemsizing = "constant", itemwidth = 35,
                    bgcolor = "rgba(255,255,255,0.9)",
                    bordercolor = "#cccccc", borderwidth = 1),
      annotations = list(list(
        text = paste0("<b>Source:</b> CEIC Database; Index, 2024 = 100",
                     ifelse(has_reference, paste0("; Black line = ", ref_label), "")),
        x = 1, y = -0.25, xref = "paper", yref = "paper", showarrow = FALSE,
        font = list(family = "Arial", size = 9, color = "#666666"),
        xanchor = "right", align = "right"
      )),
      xaxis = list(tickfont = list(size = 9), gridcolor = "#f0f0f0", tickangle = -45),
      yaxis = list(tickfont = list(size = 9), gridcolor = "#f0f0f0", zerolinecolor = "#cccccc"),
      hoverlabel = list(bgcolor = "white", bordercolor = "#cccccc", font = list(size = 10, family = "Arial")),
...
```{r}
# Level 0: Aggregated indices
create_cpi_plot(cpi_h, plot_level = 0)

# Level 1: Major groups with Overall CPI reference
...
create_cpi_plot(cpi_h, plot_level = 3, category = "FFBSS")

# Custom parameters
create_cpi_plot(cpi_h, plot_level = 2, category = "Food",
                start_date = "2022-01-01", end_date = "2025-12-31",
                ref_line_color = "darkred", ref_line_width = 1.5)
...
#|code-fold: true
#|ccode-summary: "SHOW"
# Get FFBSS subgroup as reference (level 2)
food_FFBSS <- cpi_h %>%
  filter(level == 2 & series == "Food Excl Food & Beverage Serving Services (FFBSS)")

# Get FFBSS detailed classes (level 3)
FFBSS_sub <- cpi_h %>%
  filter(group == "FFBSS" & level == 3)
...
p <- FFBSS_sub %>%
  ...
  labs(
    title = "FFBSS CPI: Subgroup vs Detailed Classes, 2020-2025",
    subtitle = "Black line = FFBSS (Food Excl F&B Serving Services)",
    x = "Date",
    y = "CPI Index",
    color = "FFBSS Detailed Classes"
  ) +  
  scale_x_date(
    limits = c(as.Date("2020-01-01"), as.Date("2026-01-01")),
    breaks = seq(as.Date("2020-01-01"), as.Date("2026-01-01"), by = "6 months"),
...
ggplotly(p) %>%
  layout(
    title = list(
      text = "FFBSS CPI: Subgroup vs Detailed Classes, 2020-2025",
      font = list(size = 16, family = "Arial", color = "#2c3e50"),
      x = 0.5,
      xanchor = "center"
    ),
    ...
    legend = list(
      ...
      x = 0.5,
      xanchor = "center",
      font = list(family = "Arial", size = 8, color = "#333333"),
      itemsizing = "constant",
      itemwidth = 30,
      bgcolor = "rgba(255,255,255,0.9)",
      bordercolor = "#cccccc",
    ...
    annotations = list(
      list(
        text = "<b>Source:</b> CEIC Database; Index, 2024 = 100; Black line = FFBSS",
        x = 1,
        y = -0.25,
        xref = "paper",
        yref = "paper",
...
make_cpi_plot_improved(cpi_h, level_val = 3, 
      category_filter = "Food",group_filter = "FFBSS",interactive = FALSE, facet_ncol = 4)
```
...
```{r}
make_cpi_plot_improved(cpi_h, level_val = 3, category_filter = "Food",group_filter = "FBSS",interactive = FALSE, facet_ncol = 2)
```
...
category_filter = "Clothing & Footwear", interactive = FALSE)
```
...
```{r}
make_cpi_plot_improved(cpi_h, level_val = 2,
category_filter = "Housing & Utilities",interactive = FALSE)
```
